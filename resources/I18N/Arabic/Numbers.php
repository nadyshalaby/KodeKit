...
class I18N_Arabic_Numbers {
    ...
    protected function writtenBlock($number) {
        ...
        if ($number > 99) {
            ...
            if ($hundred == 200) {
                array_push(
                        $items, $pre . $this->_individual[$hundred][$this->_format]
                );
            } else {
        ...
        if ($number != 0) {
            if ($this->_order == 2) {
                if ($number <= 10) {
                    ...
                    array_push(
                            $items, 'ال' . $this->_ordering[$ones][$this->_feminine]
                    );
                    array_push(
                            $items, 'ال' . $this->_individual[$tens][$this->_format]
                    );
                }
            } else {
                if ($number == 2 || $number == 12) {
                    array_push(
                            $items, $this->_individual[$number][$this->_feminine][$this->_format]
                    );
                } elseif ($number < 20) {
                    array_push(
                            $items, $this->_individual[$number][$this->_feminine]
                    );
                } else {
                    $ones = $number % 10;
                    $tens = floor($number / 10) * 10;
                    ...
                    if ($ones == 2) {
                        array_push(
                                $items, $this->_individual[2][$this->_feminine][$this->_format]
                        );
                    } elseif ($ones > 0) {
                        array_push(
                                $items, $this->_individual[$ones][$this->_feminine]
                        );
                    }

...
class I18N_Arabic_Transliteration {
    ...
    public static function en2ar($string, $locale = 'en_US') {
        ...
        foreach ($words as $word) {
            $word = preg_replace(
                    self::$_en2arPregSearch, self::$_en2arPregReplace, $word
            );
            $word = str_replace(
                    self::$_en2arStrSearch, self::$_en2arStrReplace, $word
            );
    ...
    public static function ar2en($string, $standard = 'UNGEGN') {
        ...
        foreach ($words as $word) {
            ...
            if ($standard == 'UNGEGN+') {
                $temp = str_replace(
                        self::$_diariticalSearch, self::$_diariticalReplace, $temp
                );
                ...
                $temp = str_replace(
                        self::$_rjgcSearch, self::$_rjgcReplace, $temp
                );
                ...
                $temp = str_replace(
                        self::$_sesSearch, self::$_sesReplace, $temp
                );
            } else if ($standard == 'ISO233') {
                $temp = str_replace(
                        self::$_iso233Search, self::$_iso233Replace, $temp
                );
            }

            $temp = preg_replace(
                    self::$_ar2enPregSearch, self::$_ar2enPregReplace, $temp
            );
            $temp = str_replace(
                    self::$_ar2enStrSearch, self::$_ar2enStrReplace, $temp
            );
            $temp = preg_replace(
                    self::$_arFinePatterns, self::$_arFineReplacements, $temp
            );
